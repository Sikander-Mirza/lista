...
import { Eye, EyeOff, CheckCircle, Clock, ArrowRight, X } from "lucide-react";
import Navbar from "../../Components/Navbar/Navbar";
import { Helmet } from "react-helmet-async";

// ─────────────────────────────────────────────────────────────
// WelcomeAccessModal
// Shown AFTER successful registration or Google login.
// Explains Founding Investor Access approval process.
// User must dismiss it before being redirected.
// ─────────────────────────────────────────────────────────────

const WelcomeAccessModal = ({ isOpen, onContinue, userName = "" }) => {
  ...
  return (
    ...
    >
      {/* No onClick on backdrop — user MUST read and click continue */}
      <div
      ...
      >
        ...
        <div className="h-2 w-full bg-[#703bf7] rounded-t-2xl" />
        ...
        <div className="px-6 py-6 sm:px-8 sm:py-7 flex flex-col gap-5">
          ...
          <div className="flex flex-col items-center text-center gap-3">
            <div
              className="grid h-16 w-16 place-items-center rounded-full
                          bg-violet-100 ring-4 ring-violet-50"
            >
              <svg
                width="32"
                height="32"
            ...
            <div>
              <h2 className="font-Urbanist text-xl sm:text-2xl font-bold text-gray-900">
                {userName ? `Welcome, ${userName}! 🎉` : "Account Created! 🎉"}
              </h2>
          ...
          <div className="flex flex-col gap-3">
            <p className="font-Inter text-sm font-semibold text-gray-800">
              About Founding Investor Access
            </p>
            ...
            >
              ...
              <p className="font-Inter text-xs sm:text-[13px] text-violet-700 font-medium leading-5">
                Founding Investor Access unlocks private messaging, direct
                investor-to-investor communication, and early access to
                off-market deals — all at no cost.
              </p>
            </div>
            ...
            <div className="flex flex-col gap-2 mt-1">
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="font-Inter text-xs sm:text-sm text-gray-700 font-medium">
                  <span className="font-semibold">Step 1 done —</span> Free
              ...
              <div className="flex items-start gap-3">
                ...
                <p className="font-Inter text-xs sm:text-sm text-gray-700 font-medium">
                  <span className="font-semibold">Step 2 —</span> Request
                  Founding Investor Access from your dashboard.
                </p>
              </div>
              ...
              <div className="flex items-start gap-3">
                ...
                >
                  <ArrowRight size={14} className="text-[#703bf7]" />
                </div>
                <p className="font-Inter text-xs sm:text-sm text-gray-700 font-medium">
                  <span className="font-semibold">Step 3 —</span> We review
                  your application and email you within{" "}
            ...
            >
              ...
              <p className="font-Inter text-[11px] sm:text-xs text-amber-700 font-medium leading-5">
                <span className="font-semibold">Do not create another account</span>{" "}
                if you already have one. Contact us and we'll upgrade your
          ...
          <p className="font-Inter text-[11px] text-gray-400 text-center -mt-2">
            You can request Founding Investor Access from your dashboard
            at any time.
          </p>
        </div>
      </div>
...
const AgreementModal = ({ isOpen, loading, onClose, onConfirm }) => {
      ...
      >
        ...
        <div className="px-5 sm:px-6 pt-5 pb-3 overflow-y-auto">
          ...
          <div className="rounded-xl border border-red-400 bg-red-50 p-4">
            ...
            <ul
              className="mt-3 list-disc space-y-1.5 pl-5 font-Inter
                          text-xs sm:text-sm text-red-700/90 font-medium"
            >
          ...
          <div className="mt-4 rounded-xl border border-blue-200 bg-blue-50 p-4">
            ...
            <ul
              className="mt-2 space-y-1 pl-1 font-Inter
                          text-xs sm:text-[13px] text-blue-700 font-medium"
            >
        ...
        <div
          className="mt-2 flex items-center justify-end gap-3
                      ...
                      px-5 sm:px-6 py-4 rounded-b-2xl"
        >
          <button
            onClick={onClose}
            className="font-Urbanist text-sm font-semibold inline-flex
                       items-center justify-center rounded-lg border
                       border-slate-300 bg-white px-4 py-2.5 text-slate-700
                       hover:bg-slate-50 focus:outline-none
                       focus:ring-2 focus:ring-slate-200 cursor-pointer"
          >
            Cancel
          </button>
          ...
          <button
            ...
            onClick={onConfirm}
            className="font-Urbanist text-sm font-semibold inline-flex
                       items-center justify-center rounded-lg bg-[#703bf7]
                       px-4 py-2.5 text-white hover:opacity-95
...
// ─────────────────────────────────────────────────────────────
// RequestAccessModal — exported so Pricing page can use it
// ─────────────────────────────────────────────────────────────
export const RequestAccessModal = ({ isOpen, onClose }) => {
  ...
  const handleRequest = async () => {
    try {
      ...
      if (status === 409) {
        setSubmitted(true); // already requested – idempotent
      } else {
        alert(
...
const Register = () => {
  ...
  // Agreement modal (shown BEFORE API call)
  const [showModal, setShowModal] = useState(false);

  // Welcome modal (shown AFTER successful registration)
  const [showWelcomeModal, setShowWelcomeModal] = useState(false);

  // Stores the first name to personalise the welcome modal
  const [registeredName, setRegisteredName] = useState("");

  // Stores the redirect path – determined after registration
  // but only executed after user dismisses welcome modal
  const [pendingRedirect, setPendingRedirect] = useState("");

  // Pending data while agreement modal is open
  ...
  // ─────────────────────────────────────────────────────────
  // Welcome modal continue handler
  // Only navigates AFTER user reads and dismisses the popup
  // ─────────────────────────────────────────────────────────
  ...
  // ─────────────────────────────────────────────────────────
  // STEP 2: Google button callback → open agreement modal
  // ─────────────────────────────────────────────────────────
  ...
  const registerWithEmail = async (data) => {
    ...
    try {
      ...
      // Email users go to /verify-otp after dismissing modal
      setRegisteredName(data.FirstName);
    ...
    try {
      ...
      if (profile_complete) {
        localStorage.setItem(
          "ProfileComplete",
          JSON.stringify(profile_complete)
        );
      }

      // ── Show welcome modal instead of navigating directly ──
      // Determine where to send them after they dismiss the modal
      const redirectPath = profile_complete
        ? "/admin"
        : "/admin/account-setting";

      setRegisteredName(userData.given_name || "");
  ...
  return (
    <>
      <Helmet>
        <title>
          Register for Newlista's Investor Network | Off‑Market Deals
        </title>
        <meta
          name="description"
      ...
      <div className="md:flex">
        ...
        >
          ...
          <div>
            <h1
              className="font-Poppins font-bold text-[32px]
                          max-[380px]:text-[28px] sm:text-[35px]
                          md:text-[38px] lg:text-[34px]"
            >
              Join the Investor‑Only Real Estate Network
            </h1>
            <p
              className="font-Urbanist text-Paracolor font-semibold
                          text-[13px] sm:text-[13.5px] lg:text-[15px] lg:pl-2"
            >
          ...
          {/* ── Already-have-account warning banner ── */}
          <div
            className="rounded-xl border border-violet-200 bg-violet-50
          ...
          >
            ...
            <div>
              ...
              <p
                className="font-Inter text-[11px] sm:text-xs text-violet-600
                            mt-0.5 font-medium"
              >
                To access the investor network, contact us and we will help
                activate Founding Investor Access for your existing account.
              </p>
            </div>
          </div>
          ...
          >
            ...
            <span className="relative">
              ...
              <button
                ...
                className="absolute right-4 text-[#747474] top-[46px]
            ...
            <div className="mt-1">
              <button
                type="submit"
                disabled={loading}
                className={`bg-PurpleColor w-full h-11 cursor-pointer mt-3
                            text-white font-Urbanist rounded-[6px] font-bold
            ...
            <div className="flex justify-center items-center gap-3 mt-2">
              ...
              <p
                className="font-Urbanist text-Paracolor font-semibold
                            text-[15px] sm:text-[16px] text-center
                            whitespace-nowrap"
              >
                or continue with
              </p>
          ...
          <p className="text-xs text-gray-500 mt-2 leading-5">
            ...
            networking platform for real estate investors. We do not broker
            deals, provide investment advice, or vet listings or users. All
            users are responsible for their own due diligence. Use of this
            platform is at your own risk.
          </p>
        </div>
      </div>

      {/* ── Agreement modal (before API call) ── */}
      <AgreementModal
        isOpen={showModal}
        loading={loading}
      ...
      {/* ── Welcome / Founding Investor explanation modal ──
           Fires AFTER successful registration or Google login.
           User must dismiss it — no backdrop click to close.
           Navigation only happens after they click "Go to Dashboard". */}
      <WelcomeAccessModal
